export const meta = {
  ...
  phases: [
    ...
    { title: 'Token预扫', detail: '精确统计全部开场白 Token 数（cl100k_base），供叙事学审查使用' },
...
对于每个角色子目录，确认其包含的四个文件（{名}.json、{名}_zh.json、{名}简介.md、{名}开场白.md）是否存在；异常角色卡目录按实际文件核对，不强制四件套。
如有缺失文件，标记出来。
...
然后用 node -e 运行对应的 js-tiktoken cl100k_base 计数逻辑，输出每个角色的 JSON 总计 Token 和开场白 Token。
...
## 精确 Token 数据（由 tiktoken cl100k_base 脚本实测，直接使用，禁止重新估算）
${tokenData}
...
- 节奏控制：叙述与对话的比例、段落长度、信息密度是否适宜。使用上方 Token 数据判断是否超出 900-1100 区间——**禁止自行估算 Token 数，只引用已提供的精确数据**

## Token 标准
- 达标区间：900-1100（cl100k_base 编码）
- 低于 900：信息密度不足
- 高于 1100：需要精简
