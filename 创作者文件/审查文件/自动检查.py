...
def get_token_counts():
    ...
    def segment_tokens(data):
        """拆分 JSON 为八段并返回各段 token 及总计"""
        p = data.get("char_persona", {})
        app = p.get("appearance", {})

        # ① 外观
        seg1 = {k: p.get(k, "") for k in ["gender", "age"]}
        seg1.update({k: app.get(k, "") for k in ["height", "weight", "hair", "face", "clothing", "weapon"]})
        t1 = len(enc.encode(json.dumps(seg1, ensure_ascii=False)))

        # ② 概述
        seg2 = {"char_description": data.get("char_description", {})}
        t2 = len(enc.encode(json.dumps(seg2, ensure_ascii=False)))

        # ③ 性格
        seg3 = {"char_personality": data.get("char_personality", {})}
        t3 = len(enc.encode(json.dumps(seg3, ensure_ascii=False)))

        # ④ 经历
        seg4 = {"char_background": data.get("char_background", {})}
        t4 = len(enc.encode(json.dumps(seg4, ensure_ascii=False)))

        # ⑤ 基础能力
        seg5 = {"char_basic_abilities": data.get("char_basic_abilities", {})}
        t5 = len(enc.encode(json.dumps(seg5, ensure_ascii=False)))

        # ⑥ 特殊能力
        seg6 = {"char_special_abilities": data.get("char_special_abilities", {})}
        t6 = len(enc.encode(json.dumps(seg6, ensure_ascii=False)))

        # ⑦ 角色关系
        seg7 = {"char_relationships": data.get("char_relationships", {})}
        t7 = len(enc.encode(json.dumps(seg7, ensure_ascii=False)))

        # ⑧ 对话示例
        seg8 = {"char_dialogue_examples": data.get("char_dialogue_examples", [])}
        t8 = len(enc.encode(json.dumps(seg8, ensure_ascii=False)))

        total = t1 + t2 + t3 + t4 + t5 + t6 + t7 + t8
        return [t1, t2, t3, t4, t5, t6, t7, t8, total]

    print("=" * 60)
    print("JSON Token 计数（八段式 o200k_base）")
    print("=" * 60)

    violations = []
    SEG_NAMES = ["外观", "概述", "性格", "经历", "基础能力", "特殊能力", "角色关系", "对话示例", "总计"]

    for f in sorted(glob("角色卡/**/*.json", recursive=True)):
        d = os.path.dirname(f)
        ...
        if f.endswith("_zh.json"):
            continue
        # 跳过非角色卡目录
        if not any(dd in d for dd in ["协会狩灵角色卡","事务所狩灵角色卡","游魂角色卡","罪灵角色卡","锈钟角色卡","工坊狩灵角色卡","四色音角色卡"]):
            continue
        with open(f, "r", encoding="utf-8") as fh:
            data = json.load(fh)
        vals = segment_tokens(data)
        name = os.path.basename(f)

        for i, (seg, val) in enumerate(zip(SEG_NAMES, vals)):
            lo, hi = BUDGET[seg]
            if val < lo:
                violations.append(f"  {name:<25s} {seg:<6s}偏低 {val:4d} (下限{lo})")
            elif val > hi:
                violations.append(f"  {name:<25s} {seg:<6s}超标 {val:4d} (上限{hi})")
...
# ── 全角色全名扫描 ──────────────────────────────────
def scan_fullname_violations():
    """扫描 JSON 中是否出现了其他有角色卡的人物的全名（应使用一般称呼）"""
    print()
    print("=" * 60)
    print("全名使用违规扫描")
    print("=" * 60)

    # 1. 收集所有角色卡的全名映射（排除单字名，避免 substring 误报）
    fullnames = {}  # char_name -> fullname
    for f in sorted(glob("**/*.json", recursive=True)):
        d = os.path.dirname(f)
        if "角色关系网" in d or "成品世界书" in d:
            continue
        try:
            with open(f, "r", encoding="utf-8") as fh:
                data = json.load(fh)
            ...
            continue
        cn = data.get("char_name", "")
        cfn = data.get("char_fullname", "")
        if cn and cfn and len(cfn) >= 2 and cfn != cn:  # 排除单字全名 + 全名=一般称呼(无姓)
            fullnames[cn] = cfn
    ...
    for f in sorted(glob("**/*.json", recursive=True)):
        ...
        own_name = data.get("char_name", "")
        own_fullname = fullnames.get(own_name, "")
        ...
        for target in scan_targets:
            ...
            if not isinstance(val, str):
                continue
            for cn, cfn in fullnames.items():
                if cn == own_name:
                    continue
                if cfn in val:
                    violations.append(
                        f"  {own_name:6s} {target:40s} 含全名 \"{cfn}\" 应为 \"{cn}\""
                    )

        # 检查关系字段的值（key 已在花坂扫描中覆盖，这里扫 value）
        rels = data.get("char_relationships", {})
        for rkey, rval in rels.items():
            if not isinstance(rval, str):
                continue
            for cn, cfn in fullnames.items():
                if cn == own_name or cn == rkey:
                    continue
                if cfn in rval:
                    violations.append(
                        f"  {own_name:6s} char_relationships.{rkey:12s} 含全名 \"{cfn}\" 应为 \"{cn}\""
                    )
