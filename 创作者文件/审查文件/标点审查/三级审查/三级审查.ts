...
// 格式错误由自动检查.ts 捕获，不应进入三步法审查。
const SKIP_FIELDS: RegExp[] = [
  /^char_name$/, /^char_fullname$/, /^char_alias$/,
  /^char_identity$/, /^char_rank$/, /^char_faction$/, /^char_status$/,
  /^char_persona\.(gender|age)$/,
  /^char_persona\.appearance\.(height|weight|hair|face|features|clothing|weapon|form)$/,
  /^char_basic_abilities\./,
  /^char_special_abilities\./,
  /^char_personality\.(core_traits|strengths|flaws|quirks)$/,
];

function shouldSkipField(fieldPath: string): boolean {
  return SKIP_FIELDS.some(r => r.test(fieldPath));
}
...
function commaDeleteTest(textBefore: string, textAfter: string, fullContext: string): [boolean | null, string, string] {
  ...
  // 规则 5：后句有显式主语
  if (SUBJECT_STARTERS_V2.test(textAfter.trimStart())) {
  ...
  // 规则 9 已移除：中文"的"有判断句标记/关系从句/名词化/领属四种语法功能，
  // 机械检测"的+逗号"=可删的准确率不足10%，误报率过高。
...
function periodReplaceTest(
  ...
  // 时间标记词
  if (/^(到后来|此后|在此之后|一年了|目前|如今|现在|这时|那时|那天|第二天|次日|之后|后来|这[一二三四五六七八九]年)/.test(afterTrimmed)) {
    return [false, '高', '硬止：后句为时间切换'];
  }
  // 后句引入全新实体（人名/角色名/组织名）+ 前句主语不是此实体
  const newCharMatch = afterTrimmed.match(/^([一-鿿]{2,4})(?:的|，|。|\s|$)/);
  if (newCharMatch) {
    const newSubj = newCharMatch[1];
    const beforeSubjMatch = textBefore.trimEnd().match(/([一-鿿]{2,4})(?:的|，|。|\s|$)/);
    // 如果后句主语是全新人物（不在前句中出现），且前句主语是另一个实体 → 场景切换
    if (!textBefore.includes(newSubj) && beforeSubjMatch && !textBefore.includes(beforeSubjMatch[1] + newSubj)) {
      // 两句话各有不同主语，很可能是场景/视角切换
      // 仅在前句为环境/氛围描述时触发（避免过度过滤）
      if (textBefore.length > 50 && !/^(他|她|它|我|你)/.test(textBefore.trimStart())) {
        return [false, '高', '硬止：后句引入新主语，前句为长场景描述'];
      }
    }
  }

  if (SUBJECT_STARTERS_V2.test(afterTrimmed)) {
    const afterStart = textAfter.trimStart();
    // 提取后句主语词，分类处理
    const pronSubj = afterStart.match(/^(他|她|它)/);
    if (pronSubj) {
      // 代词主语——可能是回指前句人物，保留标记
      return [null, '中', '后句代词开头，可能共享主语'];
    }
    // 名词短语主语——检查是否与前句内容重叠
    const nounSubj = afterStart.match(/^([一-鿿]{1,4})的/);
    if (nounSubj) {
      const beforeTail = textBefore.trimEnd().slice(-30);
      if (beforeTail.includes(nounSubj[1])) {
        return [null, '中', '后句主语词出现在前句中，可能话题延续'];
      }
    }
    // 新主语，不标记
    return [false, '', ''];
...
function checkPunctuation(
  ...
  opts: { ambiguousComma?: boolean; mdContext?: string; isResponse?: boolean; isFiction?: boolean } = {}
): Result[] {
  const results: Result[] = [];
  const ranges = findQuoteRanges(text);
  const sentences = extractSentences(text);
  const { ambiguousComma = false, mdContext, isResponse = false, isFiction = false } = opts;
  ...
  while ((sm = sRe.exec(text)) !== null) {
    ...
    const right = text.substring(pos + 1, sent.end).replace(/[。！？\n]+$/, '');
    const [isCorrect, confidence, reason] = semicolonSwapTest(left, right);
    if (isCorrect === false && confidence !== '') {
      results.push({ file: relpath, field: fieldName, symbol: '；',
        original: sent.text.trim(),
  ...
  while ((colm = colRe.exec(text)) !== null) {
    ...
    if (mdContext) {
      ...
      // 跳过 markdown 标题中的冒号（如 "## 发条：暴力"）
      if (/^#{1,4}\s/.test(line)) continue;
    }
    // 跳过行内元数据冒号（如段落中的"威胁等级：低。"、"类别：恶魔"）
    if (/(?:威胁等级|类别|监控建议)：/.test(text.substring(Math.max(0, pos - 6), pos + 1))) continue;
    // world info 文件是参考文档，不适用叙述标点规则
    if (relpath.includes('world info')) continue;
    ...
    for (const { pos, char, isBad, confidence, reason } of checkExclamQuestion(text, ranges, isResponse)) {
      if (isBad === false) {
        ...
        results.push({ file: relpath, field: fieldName, symbol: char,
          ...
          test: `出现 ${char}`,
          suggestion: '违规', confidence, reason, note: '' });
      }
    }
  }
...
function processMdFile(filepath: string, relpath: string): Result[] {
  ...
  const isFiction = relpath.startsWith('小说化内容');
  return checkPunctuation(text, relpath, '正文', { ambiguousComma: true, mdContext: text, isFiction });
}
...
function processJsonFile(filepath: string, relpath: string): Result[] {
  ...
  for (const [fieldPath, text] of extractChineseStrings(data, '', 5)) {
    if (shouldSkipField(fieldPath)) continue;
